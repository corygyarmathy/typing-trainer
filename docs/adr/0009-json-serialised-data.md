...
- [OpenAPI assumes JSON or YAML](https://spec.openapis.org/oas/v3.1.0.html#format). `oapi-codegen` doesn't generate Gob handlers, and the spec stops being executable.
...
- **Gob.** Rejected:
  - Gob is Go-only. The moment a Rust client, a `curl` probe, a Postman test, a JavaScript browser client, or a colleague's Python script wants to talk to your API, they can't. Even though your TUI is Go today, you don't want to weld that constraint into the contract.
  - Gob isn't introspectable. A reviewer can't `curl /api/v1/lessons/next | jq` to see what your API returns. They'd have to write a Go client first. Adds unnecessary friction.
