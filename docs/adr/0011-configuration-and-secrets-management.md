...
- **There is a live demo on a homelab host.** Production is real, not hypothetical, so the dev/prod boundary has to be enforced rather than trusted.
- **Builds should be declarative and CI-friendly.** A clone-and-run quickstart is a stated goal of the README, and CI must be able to stand the stack up with no manual secret provisioning.
- **Two deployment substrates.** The same binary is run locally via Docker Compose and in production via a NixOS module. Neither may become load-bearing for the application itself - Nix in particular must stay optional (see the Dockerfile, which builds without it).

The recurring tension is between "as declarative as possible" and "secrets". A secret that is fully declared in version control is, by definition, leaked.
...
1. **The contract** lives in `internal/platform/config`. It defines which variables exist, their defaults, and their validation rules. This is the only canonical source.
...
Development and CI use weak, clearly-labelled placeholder secrets committed in `deploy/docker/compose.yaml` and the flake dev shell. This is acceptable because those values protect nothing and the production guard rejects them.
...
- Required variables must be documented or discovered by a (deliberately loud) startup failure. Mitigated by accumulating all errors in one message and listing variables in `compose.yaml` and the README. Secret rotation requires a process restart. Acceptable at this scale.
